...
(function($) {
    $.contextMenu = {
        ...
        createMenu: function(menu, cmenu) {
            ...
            //$YG+
            var id = cmenu.trigger_id ? 'id="context-menu-for-' + cmenu.trigger_id + '"' : "";
            if (cmenu.trigger_id) {
                $("#context-menu-for-" + cmenu.trigger_id).remove(false, false);
            }
            var $t = $('<table ' + id + ' cellspacing=0 cellpadding=0></table>').click(function() {
                cmenu.hide();
                return false;
        ...
        createShadow: function(cmenu) {
            //$YG+
            var id = cmenu.trigger_id ? 'id="shaddow-for-' + cmenu.trigger_id + '"' : "";
            if (cmenu.trigger_id) {
                $("#shaddow-for-" + cmenu.trigger_id).remove(false, false);
            }
            cmenu.shadowObj = $('<div '+id+' class="' + cmenu.shadowClass + '"></div>').css({display:'none',position:"absolute", zIndex:9998, opacity:cmenu.shadowOpacity, backgroundColor:cmenu.shadowColor });
            //$YG-
        ...
        show: function(t, e) {
            ...
            if (cmenu.beforeShow() !== false) {
                ...
                $c.css({top:pos.y + "px", left:pos.x + "px", position:"absolute",zIndex:9999})[cmenu.showTransition](cmenu.showSpeed, ((cmenu.showCallback) ? function() {
